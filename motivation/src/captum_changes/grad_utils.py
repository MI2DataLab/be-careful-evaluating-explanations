...
def _neuron_gradients(
    ...
    gradient_neuron_selector: Union[
        int, Tuple[Union[int, slice], ...], Callable
    ],
) -> Tuple[Tensor, ...]:
    with torch.autograd.set_grad_enabled(True):
        gradient_tensors = []
...
def _forward_layer_distributed_eval(
    ...
    """
    saved_layer: Dict[Module, Dict[device, Tuple[Tensor, ...]]] = defaultdict(
        dict
    )
    lock = threading.Lock()
...
def _forward_layer_eval_with_neuron_grads(
    ...
    """
    grad_enabled = (
        True if gradient_neuron_selector is not None else grad_enabled
    )
    ...
    if gradient_neuron_selector is not None:
        ...
        if isinstance(layer, Module):
            return _gather_distributed_tensors(
                saved_layer[layer], key_list=key_list
            )
        else:
            return [
...
def compute_layer_gradients_and_eval(
    ...
    with torch.autograd.set_grad_enabled(True):
        ...
        all_layers: List[Module] = (
            [layer] if isinstance(layer, Module) else layer
        )
        grad_inputs = tuple(
            layer_tensor
            for single_layer in all_layers
        ...
        #! Change here retain_graph=True
        saved_grads = torch.autograd.grad(
            torch.unbind(output), grad_inputs, retain_graph=retain_graph
        )
        ...
        for single_layer in all_layers:
            ...
            if output_fn is not None:
                curr_saved_grads = [
                    output_fn(curr_saved_grad)
                    for curr_saved_grad in curr_saved_grads
                ]
